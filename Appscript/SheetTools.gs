...
function getNumOfName()
{
  console.log(getNumOfName(East));
}

//Takes a number and gives the associated region name.
function getNumOfName(strCheck)
{
  var sheetActive = SpreadsheetApp.getActiveSpreadsheet();
  var serverCheckPoint = sheetActive.getSheetByName("Facility and Environmental");
  var regionsNumRange = serverCheckPoint.getRange("A:A").getValues()
  var regionsNameRange = serverCheckPoint.getRange("B:B").getValues()
  for(i = 0; i < regionsNameRange.length; i++)
  {
    if(String(strCheck) == regionsNameRange[i][0])
    {
      return regionsNumRange[i][0]
    }
  }
}
